      ...
      <div className="myWrapper h-[inherit] flex justify-between gap-2 items-center">
        ...
        <div className="header__lang-login basis-1/6 flex gap-x-2 h-fit items-center">
          ...
          <div className="header__lang ml-auto flex gap-2 basis-1/2 justify-end">
            <p className={`${styles.language} hover:cursor-pointer ${language==='РУС' ? `${styles.chosen} text-[#662D91]`: 'text-[rgba(102,_45,_145,_0.6)]'}`} onClick={() => setLanguage('РУС')}>РУС</p>
            <p className={`${styles.language} hover:cursor-pointer ${language==='КЫРГ' ? `${styles.chosen} text-[#662D91]` : 'text-[rgba(102,_45,_145,_0.6)]'}`} onClick={() => setLanguage('КЫРГ')}>КЫРГ</p>
          </div>
        </div>
      </div>
    </header>
